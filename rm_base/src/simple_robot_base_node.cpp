...
namespace rm_base
{
    ...
    SimpleRobotBaseNode::SimpleRobotBaseNode(const rclcpp::NodeOptions &options)
    {
        ...
        //参数：serial_recv/serial_send 串口收发开关
    ...
    void SimpleRobotBaseNode::listen_loop()
    {
        ...
        while (rclcpp::ok())
        {
            if(this->SerialRecv)
            {
                // RCLCPP_INFO(node_->get_logger(), "RECV ON");
                if (this->packet_tool_->recv_packet(packet))
                {
                    ...
                    if (cmd == (unsigned char)protocol_example::GimbalAngleControl)
                    {
                        ...
                        if(this->debug)
                        {
                            ...
                            RCLCPP_INFO(node_->get_logger(), "RECV-PITCH packet[%d]: '%f'", 3, Rpitch);
                            RCLCPP_INFO(node_->get_logger(), "RECV-YAW packet[%d]: '%f'", 7, Ryaw);
